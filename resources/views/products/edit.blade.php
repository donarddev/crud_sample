...
<div class="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 py-12">
  <div class="container mx-auto px-6">
    <div class="max-w-3xl mx-auto bg-gradient-to-br from-gray-900/70 to-gray-800/60 border border-cyan-500/10 rounded-2xl p-8 shadow-2xl">
      ...
      <h1 class="text-3xl font-extrabold text-white mb-4">Edit Product</h1>
      <p class="text-sm text-gray-300 mb-6">Update product details.</p>
      ...
      <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <textarea 
            name="description" 
            rows="4" 
            class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-gray-200 focus:ring-2 focus:ring-cyan-400 focus:outline-none">{{ old('description', $product->description) }}</textarea>
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <button 
            type="submit" 
            class="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-semibold hover:brightness-110 transition">
            Update
          </button>
        </div>
